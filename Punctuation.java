 ...
 * 
 * getPunctuationWords
 * variables: String to hold all punc words, int firtLetter to hold index of first letter of word
 * 	use for loop to go through every letter
 * 		if its equal to a space or the last letter put the word in check for puc
 * 			if true is returned add it to string
 * 		then add the make firstLetter equal to the last letter
 * 
 * checkForPunctuation
 * 	check every character of the word put in with a for loop
 * 		if character is equal to punctuation return true
 * 		else return false
 * 
 * printWrods
 * 	print all the words put in
 */
...
public class Punctuation
{
	...
	{
		String para = "Blood, Sweat, and Tears by Winston Churchill " +
		"May 13, 1940" +
		"Mr. Speaker:" +
	...
	public void searchIt(String text)//Calls other methods
	{
		String para = getPunctuationWords(text);
		printWords(para);
	}
	
	public String getPunctuationWords(String text)//Method to get words
	{
		String punc = "";//String to hold all words with punctuatoin
		int firstLetter = 0;
		for (int i = 0; i < text.length(); i++)//loop to find word
		{
			if(text.charAt(i) == ' ' || i+1 == text.length())//checks for space so it can find word
			{
				if( checkForPunctuation(text.substring(firstLetter, i)))//checks if word as punc
				{
					punc = punc + text.substring(firstLetter, i);//if so, adds it to String
				}
				firstLetter = i;
			}
		}
	...
	public boolean checkForPunctuation(String word)//Method to check for punctiation 
	{
		for(int i = 0; i < word.length(); i++) //For loop to go through every letter
		{
